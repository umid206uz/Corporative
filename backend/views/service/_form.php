...
$this->registerJs(<<<JS
$('input#service-title').keyup(function(){
        var Text = $(this).val();
        NewText = makeSlug(Text);
    $('input#service-url').val(NewText);    
});
$('input#service-title_ru').keyup(function(){
        var Text = $(this).val();
        NewText = makeSlug(Text);
    $('input#service-url_ru').val(NewText);    
});
$('input#service-title_en').keyup(function(){
        var Text = $(this).val();
        NewText = makeSlug(Text);
    $('input#service-url_en').val(NewText);    
})
JS
    , 3)
?>
        ...
        <?php else:?>
            <?= $form->field($model, 'picture')->widget(FileInput::classname(), 
                [
                    ...
                    'pluginOptions' => [
                        'initialPreview'=>[
                            "/backend/web/uploads/service/". $model->filename,
                        ],
                        'showRemove' => false,
