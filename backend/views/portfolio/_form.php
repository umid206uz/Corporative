...
$this->registerJs(<<<JS
$('input#portfolio-title').keyup(function(){
        var Text = $(this).val();
        NewText = makeSlug(Text);
    $('input#portfolio-url').val(NewText);    
});
$('input#portfolio-title_ru').keyup(function(){
        var Text = $(this).val();
        NewText = makeSlug(Text);
    $('input#portfolio-url_ru').val(NewText);    
});
$('input#portfolio-title_en').keyup(function(){
        var Text = $(this).val();
        NewText = makeSlug(Text);
    $('input#portfolio-url_en').val(NewText);    
})
JS
    , 3)
?>
        ...
        <?php else:?>
            <?= $form->field($model, 'picture')->widget(FileInput::classname(), 
                [
                    ...
                    'pluginOptions' => [
                        'initialPreview'=>[
                            "/backend/web/uploads/portfolio/". $model->filename,
                        ],
                        'showRemove' => false,
